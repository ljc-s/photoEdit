...
exports.main = async (event, context) => {
  ...
  const openid = wxContext.OPENID
	const photoName = event.photoName || '默认'
	const imgBcgTip = event.imgBcgTip || '自定义'
	...
	const resContent = await getContentByDataType(photoName, imgBcgTip, img, dataType, imgType, openid)
...
async function getContentByDataType (photoName, imgBcgTip, img, dataType, imgType, openid) {
	...
	const fileID = await cloudUploadFile(`tmp/imgCompose/${openid}/${dayjs().format('YYYY-MM-DD')}/${imgName}`, imgBuffer)
	db.collection('tmp-file').add({ data: { time: db.serverDate(), fileID: fileID } })
	db.collection('user-file').add({ data: { openid: openid, photoName:photoName, imgBcgTip:imgBcgTip, time: db.serverDate(), fileID: fileID } })
